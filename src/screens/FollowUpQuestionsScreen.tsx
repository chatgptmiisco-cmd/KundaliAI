...
/** Phase 7 — adaptive multi-session onboarding: a LATER re-trigger of the
 * same topic+follow-up idea `TopicFollowUpScreen` uses for the initial
 * post-signup flow, reusing the SAME `ONBOARDING_TOPICS` data and the SAME
 * `submitOnboardingContext` call (unchanged on the backend). Deliberately
 * its own small, self-contained screen rather than sharing code with
 * `TopicFollowUpScreen`/`AiProcessingScreen` — those are tightly coupled to
 * the one-shot, non-dismissible signup stack (no back button, ends by
 * refetching the whole kundali and jumping to Validation), which is the
 * wrong ending here: this screen is reachable any time from a home-screen
 * card, is normally dismissible, and just says thanks and goes back. */
export default function FollowUpQuestionsScreen() {
  ...
  const finish = async (allAnswers: string[]) => {
    ...
    try {
      ...
      showAlert(t('common.thanks'), t('followUp.thanksMessage'));
    } catch (err: any) {
